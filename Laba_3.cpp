...
int main() {
	...
	conteiner st; // object_of_conteiner_class
...
void instrumenti_PO::set_all() {
	...
	if (cin.fail()) {
		cin.clear();
		cin.ignore(32767, '\n');
		throw(exception("Vvedite_4islo!\n"));
	}
	else if (this->year < 2011 || this->year > 2021)
		throw(exception("Vveden_nepravilniy_god!\n"));
	...
	/* Version */
	cout << "Enter version (s to4koy): "; cin >> this->version;
	if (cin.fail()) {
		cin.clear();
		cin.ignore(32767, '\n');
		throw(exception("Vvedite_veshestvennoe_4islo!\n"));
	}
